---
name: naverblog-form
description: naverblog-form/SKILL.md는 완성된 본문 텍스트를 네이버 블로그 스마트에디터 ONE에 붙여넣을 수 있는 HTML 파일로 변환하는 SKILL이다. 본문에 강조와 구조를 입혀 네이버 호환 HTML로 만들어야 하는 상황에 사용한다. 사용자가 네이버 블로그 글의 서식 작업을 요청하는 상황에도 사용한다.
---
# naverblog-form
...
### Overview
naverblog-form/SKILL.md는 완성된 본문 텍스트를 네이버 블로그 스마트에디터 ONE에 붙여넣을 수 있는 HTML 파일로 변환하는 SKILL이다.


### Triggers
- 본문 텍스트를 네이버 블로그용 서식으로 변환해야 하는 상황
- 네이버 블로그에 붙여넣을 HTML이 필요한 상황
- 사용자가 네이버 블로그 글의 서식 작업을 요청하는 상황
...
### Purpose
완성된 본문 텍스트에 강조와 구조를 입혀, 네이버 블로그 스마트에디터 ONE에 붙여넣었을 때 서식이 보존되는 HTML 파일로 변환한다.
...
**블록**
- 인용구: `<blockquote>텍스트</blockquote>`. 따옴표형 한 종류만 가능하다.
- 구분선: `<hr>`.
...
- 강조와 구조는 본문을 읽고 직접 판단한다. 사용자는 별도의 서식 표시 없이 본문만 전달한다.
...
- 절제가 기본이다. 강조는 꼭 필요한 곳에만 쓴다. 색과 형광을 남발하지 않는다.
